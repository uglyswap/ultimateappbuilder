...
export class MobileAppGeneratorService {
   ...
   */
  async generateMobileApp(config: MobileAppConfig): Promise<{
    ...
    logger.info(`Generating React Native app: ${config.appName}`);
    ...
    logger.info('Mobile app generation completed!');
   ...
   * Generate package.json
   */
  private generatePackageJson(config: MobileAppConfig): string {
   ...
   */
  private generateAppJson(config: MobileAppConfig): string {
    return JSON.stringify(
      {
        expo: {
          name: config.appName,
   ...
   */
  private async generateNavigation(config: MobileAppConfig): Promise<string> {
...
Generate ONLY the navigation configuration code.`;
   ...
   */
  private async generateScreens(config: MobileAppConfig): Promise<Record<string, string>> {
    const screenTypes = [
      'Home',
      'Profile',
      'Settings',
...
Generate ONLY the screen component code.`;
   ...
   * Generate reusable components
   */
  private async generateComponents(config: MobileAppConfig): Promise<Record<string, string>> {
...
`;
   ...
   * Generate API client
   */
  private async generateApiClient(config: MobileAppConfig): Promise<string> {
...
`;
   ...
   * Generate state store
   */
  private async generateStateStore(config: MobileAppConfig): Promise<string> {
    if (config.stateManagement === 'zustand') {
...
`;
   ...
   */
  private generateProjectStructure(config: MobileAppConfig): any {
    return {
   ...
   */
  async generateIOSBuildConfig(): Promise<string> {
    return JSON.stringify(
      {
        build: {
          production: {
   ...
   */
  async generateAndroidBuildConfig(): Promise<string> {
    return JSON.stringify(
      {
        build: {
          production: {
...
export const mobileAppGeneratorService = new MobileAppGeneratorService();
