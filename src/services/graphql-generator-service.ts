...
export class GraphQLGeneratorService {
  ...
  async generateSchemaFromDescription(
    ...
    logger.info(`Generating GraphQL schema for project ${projectId}`);
...
${fields}
  ...
  private generateQueryDefinitions(queries: GraphQLQuery[]): string {
...
${queryFields}
  ...
  private generateMutationDefinitions(mutations: GraphQLMutation[]): string {
...
${mutationFields}
  ...
  private generateSubscriptionDefinitions(subscriptions: GraphQLSubscription[]): string {
...
${subscriptionFields}
   ...
   */
  async generateResolvers(schema: GraphQLSchema): Promise<string> {
    logger.info('Generating GraphQL resolvers with AI');
...
Generate ONLY the resolver code, nothing else.`;
   ...
   */
  async generateApolloServerConfig(projectName: string): Promise<string> {
...
`;
   ...
   */
  async generateDataLoaders(schema: GraphQLSchema): Promise<string> {
...
${schema.types
  .map(
    type => `    ${type.name.toLowerCase()}Loader: new DataLoader(async (ids: readonly string[]) => {
...
`;
   ...
   */
  async generateCompleteGraphQLAPI(
    projectId: string,
    description: string,
    features: string[]
  ): Promise<{
    ...
    logger.info(`Generating complete GraphQL API for project ${projectId}`);
    ...
    logger.info('GraphQL API generation completed successfully!');
   ...
   */
  async generateSubscriptionServer(): Promise<string> {
...
export const graphqlGeneratorService = new GraphQLGeneratorService();
